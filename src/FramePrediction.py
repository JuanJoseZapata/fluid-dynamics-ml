...
class NextFramePredictionModel(nn.Module):
    def __init__(self, in_channels, hidden_dim, kernel_size, num_convlstm_layers=1, num_lstmcell_layers=1, batch_first=True):
        ...
        self.convlstm_layers.append(
            ConvLSTM(in_channels, hidden_dim, kernel_size, num_lstmcell_layers, batch_first)
        )
        self.bn_layers.append(nn.BatchNorm3d(hidden_dim))
        self.relu_layers.append(nn.ReLU())

        # Additional layers (if num_convlstm_layers > 1)
        for _ in range(1, num_convlstm_layers):
            self.convlstm_layers.append(
                ConvLSTM(hidden_dim, hidden_dim, kernel_size, num_lstmcell_layers, batch_first)
            )
            self.bn_layers.append(nn.BatchNorm3d(hidden_dim))
            self.relu_layers.append(nn.ReLU())
    ...
    def forward(self, x):
        ...
        for i in range(self.num_convlstm_layers):
            # ConvLSTM: (B, C, T, H, W) → internally expects (B, T, C, H, W)
            x = self.convlstm_layers[i](x)  # Output: (B, T, C, H, W)

            # Permute for BatchNorm3d
            x = x.permute(0, 2, 1, 3, 4)  # (B, C, T, H, W)
            x = self.bn_layers[i](x)
            x = self.relu_layers[i](x)

            # Permute BACK for next ConvLSTM (unless last layer)
            if i < self.num_convlstm_layers - 1:
